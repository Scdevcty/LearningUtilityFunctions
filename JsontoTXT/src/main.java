		...
		for(int m=61;m<=78;m++)
		{
			...
			String filename="TOVOR"+Integer.toString(m)+".json";
			 File file=new File(filename);
		        ...
		        FileWriter filewriter= new FileWriter("TOVOR_STAND_GUARD_78",true);
		        	...
		        	for(int j=0;j<array.size();j++)
		        	{
		        		...
		        		if(object.getJSONObject("P1").getString("action").equals("STAND_GUARD"))
		        		{
		        	        ...
		        	        if(!s.contains(b))
		        	        {
		        	        	...
		        	        	int p2hitp1=canHit(p2action,p1action);
		        	        	fw.write(p1hp+" "+p2hp+" "+p1x +" "+p2x +" "+p1speedx+" "+p2speedx+" "+p2attack+" "+p1hitp2+" "+p2hitp1);
		        	        	fw.newLine();
		        	        }
		        		}
		        	}
